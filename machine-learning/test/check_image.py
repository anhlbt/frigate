...
def assess_expression(image):
    ...
    ## with local emotion detection 
    # emotion = face_emotion_estimator.estimate(image)
    # return emotion, ""
...
# Function to compare two frames using Structural Similarity Index (SSI)
...
def get_best_frame(video_path, output_folder="./outputs", num_frames=100, offset=30):
    os.makedirs(output_folder, exist_ok=True)
    # Extract frames from the video
    cap = cv2.VideoCapture(video_path)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    for idx in frame_indices:
        ...
        if ret:
            frames.append((idx, frame))  # Save frame index along with frame
    cap.release()
    # Filter frames based on face analysis rules
    selected_frames = []
    for idx, (frame_idx, frame) in enumerate(frames):
        ...
        for face in faces:
            ...
            if any(keyword in result for keyword in ['FACE_RIGHT', 'FACE_LEFT', 'FACE_UP', 'FACE_DOWN', 'MOUTH_OPEN', 'EYE_CLOSE']):
                valid_frame = False
                print("* " * 5, result, " *" * 5)  
                ignore_img = os.path.join(output_folder, f'{os.path.basename(video_path)}_bypass_frame_{frame_idx}.jpg')
...
def main(video_path, interval=30, threshold=0.6):
    # distinct_frames = extract_distinct_frames(video_path) # get_best_frame
    distinct_frames = get_best_frame(video_path)
    best_frame = None
    best_score = -1
    print("num frame: ", len(distinct_frames))
    for i, (frame_count, frame) in enumerate(distinct_frames):
        faces = app.get(frame)
        for face in faces:
    ...
    # if best_frame is not None:
        ...
        cv2.imwrite(f"./outputs/best_frame_with_mask_tmp_{frame_count}.jpg", annotated_frame)
        # cv2.waitKey(0)
        time.sleep(1)
        cv2.destroyAllWindows()
    # else:
    #     print("No suitable frame found in the video.")

# Run the main function
face_emotion_estimator = FaceEmotionEstimator(path=INPUT_DIR_MODEL_ESTIMATION)
video_path = '/workspaces/frigate/storage/recordings/2024-06-15/08/front-cam/37.05.mp4'
main(video_path, interval=30, threshold=0.9)
