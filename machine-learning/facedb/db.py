...
import warnings
import threading
from app.models.facial_recognition import FaceRecognizer
from typing import Any, Callable, List, Union, Tuple, Literal, Optional
...
# from arcface import FaceRecognizer  # Import the FaceRecognizer class from arcface.py

import_lock = threading.Lock()


def load_module(module: Literal["deepface", "face_recognition"], **model_kwargs: Any):
    with import_lock:
        if module == "deepface":
            global DeepFace
            ...
            recognizer = FaceRecognizer(**model_kwargs)
            recognizer._load()
            print("load module FaceRecognizor")
...
def create_arcface_embedding_func(model_name: str, cache_dir: Union[str, Path], **model_kwargs: Any) -> Callable:
    def embedding_func(image: np.ndarray) -> np.ndarray:
        faces = recognizer._predict(image) 
        # faces = recognizer._embed(image) # get embedding from aligned image (1 face)
        if faces:
            return [face['embedding'] for face in faces]  # return faces[0]['embedding']
        # else:
        #     return [0.0] * 512 # np.zeros((512,), dtype=np.float32).tolist()  # Assuming embedding size is 512
    return embedding_func

def create_arcface_extract_faces_func(model_name: str, cache_dir: Union[str, Path], **model_kwargs: Any) -> Callable:
    def extract_faces(image: np.ndarray, is_ligned: bool = True) -> List[np.ndarray]:
        if not is_ligned:
            bboxes_faces = recognizer._detect(image)
            return [Rect(x, y, w, h) for (x, y, w, h) in bboxes_faces]
        else:
            x, y, w, h = get_image_bounding_box(image)
            return [Rect(x, y, w, h)]
...
class FaceDB():
    def __init__(
        ...
        *,
        ...
        module: Literal["deepface", "face_recognition", "arcface"] = "face_recognition",
        ...
        **kw,
        ...
        assert module in [
            ...
            "face_recognition",
            "arcface",
        ], "Supported modules are `deepface`, `face_recognition` and `arcface`."
        ...
        if embedding_func is None:
            if module == "deepface":
                ...
                self.embedding_func = create_arcface_embedding_func(
                    model_name=kw.pop("model_name", "arcface"),
                    cache_dir=kw.pop("cache_dir", None),
                    **kw,
                )
                self.extract_faces = create_arcface_extract_faces_func(
                    model_name=kw.pop("model_name", "arcface"),
                    cache_dir=kw.pop("cache_dir", None),
                    **kw,
                )
                ...
                raise ValueError(
                    "Currently only `deepface`, `face_recognition` and `arcface` are supported."
                )
    ...
    def add(
        ...
        **metadata,
        ...
        if check_similar:
            result = self.check_similar(embeddings=embedding)
            if result:
                print(
